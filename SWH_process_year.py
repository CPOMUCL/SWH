...
for mn in range(10):
    ...
    l1b_dir = l1b_root+dt_mn.strftime('%Y%m')+'_SAR/rads/'
...
# for f in range(10):
    for f in range(f_tot):
        ...
        this_path = SWH_wfm.l1b_track(files[f],lon_lims,lat_lims,'SAR')
        use = this_path.use
        if use:
        ...
        # down sampling and some filtering
            this_path.filter_for_SWH(reduce_WF=20,LE_lim=0.7)
            print('calculating SWH')
        # the main retracking routine
            this_path.get_SWH(SAREcho,[1.0,0.1,28.0],method='sig_t_A',
